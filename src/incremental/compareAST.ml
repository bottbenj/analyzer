...
let eq_instr (a: instr) (b: instr) = match a, b with
  ...
  | Asm (attr1, tmp1, Some (ci1, dj1, rk1, gt1), l1), Asm (attr2, tmp2, Some (ci2, dj2, rk2, gt2), l2) -> eq_list String.equal tmp1 tmp2 && eq_list(fun (x1,y1,z1) (x2,y2,z2)-> x1 = x2 && y1 = y2 && eq_lval z1 z2) ci1 ci2 && eq_list(fun (x1,y1,z1) (x2,y2,z2)-> x1 = x2 && y1 = y2 && eq_exp z1 z2) dj1 dj2 && eq_list String.equal rk1 rk2 && eq_list String.equal gt1 gt2(* ignore attributes and locations *)
  | Asm (attr1, tmp1, None, l1), Asm (attr2, tmp2, None, l2) -> eq_list String.equal tmp1 tmp2
